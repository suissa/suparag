...
test.describe('Métricas', () => {
  ...
  test('deve exibir gráficos', async ({ page }) => {
    ...
    await expect(page.locator('text=Tickets por Status')).toBeVisible();
    await expect(page.locator('text=Interações por Canal')).toBeVisible();
  });

  test('deve exibir resumo detalhado', async ({ page }) => {
    await page.waitForTimeout(1000);
    
    await expect(page.locator('text=Resumo Detalhado')).toBeVisible();
  });

  test('deve ter ícones nos cards', async ({ page }) => {
    await page.waitForTimeout(1000);
    
    // Verificar se existem ícones material
